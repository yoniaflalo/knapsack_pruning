...
try:
    ...
    import nn.SyncBatchNorm as SyncBatchNorm
...
def compute_time_per_layer(model, input_size, batch_size=512):
    time_total, dict_time = measure_time(model, iterations=10, input_size=input_size, batch_size=batch_size)
...
def compute_num_channels_per_layer_taylor(model, input_size, data_loader, pruning_ratio=0.5,
                                          ...
                                          use_time=False, distributed=False):
    if not (isinstance(input_size, list) or isinstance(input_size, tuple)):
        input_size = [3, input_size, input_size]
    if use_time:
...
def measure_time(module, iterations=10, input_size=224, batch_size=512, fp16=False):
...
class build_co_train_model(nn.Module):
    ...
    def __init__(self, module1, module2, gamma=0.5, only_last=False, progressive_IKD_factor=False):
